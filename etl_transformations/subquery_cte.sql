...
-- ETL transformations prepare data to be used to for analytical queries (i.e. Clean raw ingestion data, prepares fields for downstream analytics, deduplication logic, feature creation with case statements)

------------------------------------------------------------------------------


-- Ensure that for each project_id, the review_date is always after the end_date of the lastest end_date for that project_id. the query should have a subquery in the WHERE clause.
...
-- Detect invalid values within the sales table. How would you identify customers that do not meet the specified regular expreseeion pattern?
SELECT *
FROM sales
